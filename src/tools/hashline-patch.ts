  ...
  parameters: Type.Object(
    {
      patch: Type.Optional(
        Type.String({
          description:
            "Inline patch text. Mutually exclusive with `patch_file`. Must start with `*** Begin Patch` and end with `*** End Patch`. May contain multiple `*** Add File`, `*** Update File`, and `*** Delete File` sections; `*** Update File` sections may contain multiple `@@` hunks. Update hunk headers are `@@` or `@@ @<line>`; `@@ @<line>` starts searching at 1-based line `<line>` and requires the resolved match start to be at or after that line. Update hunk operations use one operation character plus a selector: ` :<text>` exact context text, `-:<text>` exact delete text, `+<text>` insert literal text, ` #<hash>` hash context, `-#<hash>` hash delete, ` ...` context range, and `-...` delete range. Prefix selectors (`^<prefix>`) are not supported yet. Do not use read-output `HASH│content` rows as patch operations. Insert lines use raw `+{text}` content; do not include hashes in `+` lines unless those hash characters are intended file content. Example:\n```\n*** Begin Patch\n*** Update File: path/to/file.txt\n@@ @120\n :start context\n ...\n #ABCD\n-...\n+replacement text\n :end context\n*** End Patch\n```"
        })
      ),
